...
prompt token 17,932개와 completion token 2,822개였고, 추정 비용은
`$0.026148`이었다.
...
첫 번째 유료 시도는 replay writing 전에 실패했다. LLM response가 parser가
integer `target_user_id`를 기대하는 optional field에 non-integer 값을 반환했기
때문이다. 조사 결과 실제 interface bug가 있었다. prompt는 integer
`target_user_id`를 요구했지만, feed line에는 `author_id`가 없고 `@handle`만
있었다.
